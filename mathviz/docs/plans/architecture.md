...
> **mathviz = { a 2D plane you draw on } × { push it through any map } × { color it by a complex field }**
...
`mathviz/src/mathviz/`) wired into the root project via `[tool.uv.sources]`. The src layout matters:
without it, running from the repo root would import the `mathviz/` *directory* as a namespace package
and shadow the installed one. Because the sub-package has its own env, **notebook/pytest tooling is run
from the repo root** (which has the jupyter/test toolchain and the editable `mathviz`).
