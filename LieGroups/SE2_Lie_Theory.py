...
def draw_frame(ax, g, color='white', label='', alpha=1.0, scale=0.4):
    ...
    if label:
        ax.text(origin[0]+0.05, origin[1]+0.05, label, color=color,
                fontsize=11, alpha=alpha)

# --- Visualize group action ---
fig, axes = plt.subplots(1, 2, figsize=(13, 5))
fig.suptitle('SE(2): Rigid Body Transformations in the Plane',
             fontsize=14, color='white', y=1.01)

# A simple shape (triangle)
triangle = np.array([[0.0, 0.3, 0.15, 0.0],
                     [0.0, 0.0, 0.3,  0.0],
                     [1.0, 1.0, 1.0,  1.0]])
...
for ax, (g, label, color, title) in zip(
    ...
    ax.set_xlim(-1.5, 2.5); ax.set_ylim(-0.8, 2.5)
...
e1 = np.array([[0, -1, 0],   # infinitesimal rotation
               [1,  0, 0],
               [0,  0, 0]], dtype=float)

e2 = np.array([[0, 0, 1],    # infinitesimal translation in x
               [0, 0, 0],
               [0, 0, 0]], dtype=float)

e3 = np.array([[0, 0, 0],    # infinitesimal translation in y
               [0, 0, 1],
               [0, 0, 0]], dtype=float)
...
fig.suptitle('Lie Algebra se(2): Integral Curves of Basis Elements\n'
...
group_elements = [
    ...
    (se2_element(np.pi/4, 1.0, 0.5), 'g = rotation + translation\n(θ=45°, v=(1,0.5))'),
]
...
for ax, (g, title) in zip(axes, group_elements):
    ...
    ax.annotate('', xy=(ux_Y, uy_Y), xytext=(0,0),
                arrowprops=dict(arrowstyle='->', color=BLUE, lw=2.5))
    ax.text(ux_Y+0.05, uy_Y+0.05, 'Y', color=BLUE, fontsize=12)

    ax.annotate('', xy=(ux_A, uy_A), xytext=(0,0),
    ...
    ax.text(-0.4, 1.7, f'ω(Y)={omega_Y:.2f}\nω(Ad_gY)={omega_A:.2f}',
...
for omega, ux, uy, desc in test_cases:
    ...
    print(f"\n{desc}")
    print(f"  ||Ad_{{exp(X)}} - exp(ad_X)||_F = {np.linalg.norm(LHS - RHS):.2e}  ✓" if match
...
for row in range(2):
    for col in range(3):
        ...
        ax.plot(ts, RHS_entries[:, i, j], '--', color=ORANGE, lw=2, label=r'$\exp(t·ad_X)$')
...
ax1.set_title('Lie bracket as failure of commutativity\n'
              'exp(sX)exp(tY) vs exp(tY)exp(sX)', color='#ccc', fontsize=11)
ax1.set_xlim(-0.5, 3.5); ax1.set_ylim(-1.0, 2.5)
...
ax1.annotate('', xy=end2, xytext=end1,
             arrowprops=dict(arrowstyle='->', color=GREEN, lw=2,
                             connectionstyle='arc3,rad=0.3'))
ax1.text((end1[0]+end2[0])/2 + 0.1, (end1[1]+end2[1])/2,
...
box_style = dict(boxstyle='round,pad=0.6', facecolor='#1a1a3e', edgecolor='#555', lw=1.5)

ax2.text(2.5, 6.5, 'SE(2)', fontsize=16, color=BLUE, ha='center',
         bbox=dict(boxstyle='round,pad=0.5', facecolor='#001133', edgecolor=BLUE, lw=2))
ax2.text(7.5, 6.5, 'se(2)', fontsize=16, color=ORANGE, ha='center',
         bbox=dict(boxstyle='round,pad=0.5', facecolor='#1a0a00', edgecolor=ORANGE, lw=2))

ax2.annotate('', xy=(6.7, 6.7), xytext=(3.3, 6.7),
             arrowprops=dict(arrowstyle='->', color=GREEN, lw=2))
ax2.text(5.0, 6.8, r'$\text{log}$', color=GREEN, fontsize=10, ha='center')


ax2.annotate('', xy=(6.65, 6.5), xytext=(3.2, 6.5),
             arrowprops=dict(arrowstyle='<-', color=PURPLE, lw=2))
ax2.text(5.0, 6.3, r'$\exp$', color=PURPLE, fontsize=10, ha='center')

rows = [
    (2.5, 4.8, BLUE,   r'$\text{Ad}_g$ : se(2) → se(2)', 'Y ↦ g Y g⁻¹', 'conjugation in group'),
    (7.5, 4.8, ORANGE, r'$\text{ad}_X$ : se(2) → se(2)', 'Y ↦ [X, Y]',  'Lie bracket'),
    (5.0, 2.8, GREEN,  'Connecting theorem:', r'$\text{Ad}_{\exp(X)} = \exp(\text{ad}_X)$', '(differentiate Ad at identity)'),
    # (2.5, 1.2, BLUE,   'Ad is a group homomorphism', 'G → GL(𝔤)', ''),
    (2.5, 1.2, BLUE,   'Ad is a group homomorphism', r'$G → \text{GL}(\mathfrak{g})$', ''),
    #(7.5, 1.2, ORANGE, 'ad is a Lie algebra homomorphism', '𝔤 → 𝔤𝔩(𝔤)', ''),
    (7.5, 1.2, ORANGE, 'ad is a Lie algebra homomorphism', r'$\mathfrak{g} → \mathfrak{gl(g)}$', ''),
]
...
ax2.text(5.0, 3.5, 'SE(2)-specific: rotation ω preserved''; translation u rotated by R',
         color='#aaa', fontsize=8.5, ha='center')
