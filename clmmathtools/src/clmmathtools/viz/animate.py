...
of the fundamental operations (see :mod:`clmmathtools.viz.maps`) each path can animate: GL(2, ℝ) has **two
...
def scrubber(
    ...
    *,
    ...
    ``width``/``height`` are the ``Image`` widget's **CSS** pixel size. A notebook frontend (VS Code,
    Jupyter) renders the image at a *CSS* size — it scales the image to the cell width regardless of
    the PNG's pixel ``size`` — so ``width``/``height`` (not ``size``) is what controls how much of the
    screen the image occupies. Set e.g. ``width=320, height=320`` to fit a narrow screen.
    """
    ...
    if width is not None:
        img.width = f"{width}px"
    if height is not None:
        img.height = f"{height}px"
...
def animate_matrix(
    ...
    **scrubber_kw,
    ...
    ``probe_shape`` (``None``, or any points ``np.asarray`` accepts — see :func:`clmmathtools.viz.maps.push`)
    is the figure carried along; it defaults to the unit square, whose image area is ``det M(t)``.

    ``color`` styles the **warped image grid** (its default is purple, matching the ``push``
    convention); pass e.g. ``color=clmmathtools.viz.GREEN`` / ``clmmathtools.viz.BLUE`` for variety. ``probe_shape_color``
    colors the probe's image separately (``None`` → follow ``color``).
    """
