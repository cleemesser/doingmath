# clmmathtools — backlog & reminders
...
- **SymPy pretty-printing in notebooks** ✅ *(delivered)*: added `clmmathtools.viz.show_expr(expr, label=None)`,
...
- **Migrate existing notebooks** (Phase 5): delete the ~10 duplicated `Plane2D` / `new_plot` helpers in
  `GeometricLinearAlgebra/` and `Geometry/`; replace with `import clmmathtools`; re-verify renders.
