...
- **Example notebooks** in `examples/` double as executable docs *and* feature tests — every cell
  ...
  ```bash
  uv run jupytext --to notebook --execute clmmathtools/examples/01_plane_and_primitives.py
  uv run jupytext --to notebook --execute clmmathtools/examples/02_maps.py
  ```
...
- **Phase 5** — migrate the `GeometricLinearAlgebra/` and `Geometry/` notebooks onto `clmmathtools`. 🚧
