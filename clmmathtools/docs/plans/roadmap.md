# clmmathtools — roadmap

`clmmathtools` visualizes the 2D / complex plane: draw on it, push it through maps, color it as a Wegert
...
- **`GeometricLinearAlgebra/`**: ✅ **all `00`–`07` migrated** onto `clmmathtools` — the inline `Plane2D`
  (vedo) and `new_plot`/k3d toolkits replaced by `clmmathtools` (thin adapters keep the original call sites
...
- ⬜ ComplexAnalysis gallery notebooks built on `clmmathtools` (the original motivation).
