...
# For a vector space $V$ endowed with an inner product. Let's consider vector $u,v \in V$ and their dual vectors $u^*,v^* in V^*$ and $L$ linear function: $V->V$:
# $$
#   v^*(v) = v \cdot v = <v,v>$$
# $$  
#   v^*(u) = v \cdot u = <v,u>
...
from mathviz import show_md as md, show_expr
...
e1,e2 = alg2.blades.e1, alg2.blades.e2  # import kingdon's defintions for standard defintion
...
newbasis_list = [[1,0], # same as e1
                 [1,1]] # length 2, at 45 degree angle to e1, e2
...
the $\{b_i\}$ basis, will transform (via a shear operation) into vector components in the standard
 basis""")
md(r"""$B\,\vec{b}_2 = """ rf"""{latex(B)}{latex(Matrix([0,1]))} = {latex(B*Matrix([0,1]))}$""")
...
w_mv = alg2.vector(np.random.normal(size=(2,)))
v_mv, u_mv, w_mv
# %% [markdown]
...
mv.show_md("$Gb^{ij} = " f"{sp.latex(Matrix(Gb_raise))}$")
...
mv.show_md(r'$ub = B^{-1} u$ to transforms compoents of $\vec{u}$ in std basis to components in new basis $b_j$')
...
w_arr_b[0] *b1_mv + w_arr_b[1]*b2_mv, w_mv # check that this recreates the original-yes
#w_arr_b, v_arr_b
...
# %%
# get back originals
mv.show_md(f'covector components $ub_i$ in basis b: {einsum(Gb_arr,u_arr_b,'i j,j->i')}')
...
md(rf"""In the b_i coordinates, the components for the dual basis are: {b1_dual_b} and {b2_dual_b}""")
Gb_raise_arr
...
# %%
(b1_dual_mv | b1_mv).e, (b1_dual_mv | b2_mv).e, (b2_dual_mv|b2_mv)

# %%
b1_dual_b.T @ Gb_arr @ b1_arr_b, b1_dual_b.T @ Gb_arr @ b2_arr_b, b2_dual_b @ Gb_arr @ b2_arr_b

# %%
b2_arr

# %%
b1_dual_b.T @ b1_dual_b, b1_dual_b.T @ b2_arr_b, b2_dual_b.T @ b2_arr_b, 
