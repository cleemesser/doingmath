...
| 2 | [`The_Antisymmetric_Part_of_DF`](The_Antisymmetric_Part_of_DF.py) | Takes §7 of notebook 1 — *"curl is a vector only when $n=3$"* — and replaces the dimension count with the object it was counting. The spin bivector $A=\tfrac12(DF-DF^{\mathsf T})$ exists in every dimension and every signature; via the real Schur form it is **$\lfloor n/2\rfloor$ rotation planes at $\lfloor n/2\rfloor$ independent speeds**, and the vector curl needs *two* coincidences at once — that a bivector in $\mathbb{R}^{n\le3}$ has only **one** plane (measured by $\operatorname{Pf}A=0$), and that a plane in $\mathbb{R}^3$ has a normal **line**. $n=5$ has the second without the first. Then **spacetime**: $F_{\mu\nu}=\partial_\mu A_\nu-\partial_\nu A_\mu$ is that same construction on the 4-potential, $6=\binom42$ is why the field has six components, and $\mathbf E$/$\mathbf B$ is the $n=3$ repackaging done inside an observer's slice — a rotor boost is applied and checked against Jackson §11.9 while $\mathbf E^2-\mathbf B^2$ and $\mathbf E\cdot\mathbf B$ (the Pfaffian again) hold still. Both wedge formalisms: an $n$-dimensional SymPy $d$/$\wedge$/$\star$, and geometric algebra where $\nabla F=\nabla\cdot F+\nabla\wedge F$ makes div and curl **two grades of one product** in identical code for $n=2,3,4,5$ and for $\mathbb{R}^{1,3}$. Closes with an aside on the octonionic **7-dimensional cross product**: real, but a rank-7 projection of a 21-dimensional space, and *not* $SO(7)$-equivariant — averaged over Haar $SO(7)$ it vanishes into the Monte-Carlo floor, so Schur still forbids a curl there. |

| 3 | [`Generalizations_of_Curl`](Generalizations_of_Curl.py) | Asks the question notebook 2 answers only for its own candidate: *what are the generalizations of curl in $\mathbb{R}^n$* — plural. The premise is that in $\mathbb{R}^3$ **seven** inequivalent properties each pin down $\nabla\times$ uniquely (all seven verified in §1), so generalizing means choosing which to keep, and the choices diverge. **Keep "$d$ on a 1-form" and there were never three operators**: $\mathbb{R}^n$ has $n$, the de Rham complex with dimensions $\binom nk$ — grad/curl/div is just the chain being short enough at $n=3$ to name every arrow, while $\mathbb{R}^4$ has *two* middle operators and neither is the curl ($d^2=0$ proven in all degrees for $n=4,5$; generalized Stokes checked on a 2-cube in $\mathbb{R}^4$). **Keep "vector in, vector out" and you get nothing**: §5 solves the equivariance condition $L(RJR^{\mathsf T})=RL(J)$ directly and finds a solution space of dimension **0 for every $n\neq3$** and exactly **1** at $n=3$ — strictly stronger than the usual $\binom n2=n$ count, which rules out only one construction; the consolation prize is that $\star dF^\flat$ becomes a vector only after $n-3$ arbitrary directions are chosen. **Keep the curl-curl identity** and you get the codifferential $\delta$ and $\Delta=d\delta+\delta d$, the one operator here that returns what it consumes in every dimension. **Keep the Poincaré lemma and you get topology**: closed-but-not-exact witnessed by the angle form ($\oint=2\pi$), then **Hodge's theorem computed** — Betti numbers of a torus, sphere and annulus obtained twice, from ranks of $\partial$ and as $\dim\ker$ of the combinatorial Hodge Laplacian, agreeing, with Euler–Poincaré falling out. Ends in a scorecard where no row scores seven. |
