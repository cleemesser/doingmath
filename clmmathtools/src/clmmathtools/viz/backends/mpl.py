"""Matplotlib backend — 2D, raster-native (the natural home for phase portraits). The default
backend.

Inline output is **SVG** by default: it stays crisp at any zoom, ships a few times smaller than
the equivalent base64 PNG, and GitHub renders it in ``.ipynb`` outputs. Pass ``format="png"`` for
raster-heavy scenes (phase portraits, ``raster``, ``field_complex``), where the SVG is only a
wrapper around an embedded PNG and so comes out *larger* — measured ~1.3x on a 300px portrait,
against ~0.33x the other way for a scene made of lines and arrows."""
...
def make_mpl_svg_inline(scene, width=430) -> str:
    """A scene -> a self-contained ``<div>`` of inline SVG, as an HTML string.

    For hosts that render HTML themselves rather than going through ``IPython.display`` —
    a reactive notebook such as marimo, a static site, an email. ``Plane.display()`` already
    emits SVG for the IPython path; reach for this only when you need the markup in hand.

    Two details the wrapper exists for, both of which matter when a scene re-renders on every
    widget tick:

    * **Inline, not an asset.** An ``<img>`` pointed at a content-hashed URL mints a fresh URL
      per frame, so the browser tears the old one down and re-fetches. The blank gap in between
      is a visible flash. Inline SVG is DOM, and swaps in the same paint as its surroundings.
    * **A reserved box.** The ``<div>`` is sized from ``scene.view``'s aspect ratio, so the row
      cannot collapse to zero height and reflow while the image is in flight.

    ``scene`` must support ``.save(buffer, format="svg")`` and expose ``.view.size`` — true of
    ``Plane`` and ``Space3D`` on the matplotlib backend. The vedo backend renders raster and
    will not produce usable SVG here.
    """
...
class MatplotlibBackend(Backend):
    ...
    def _figure(w, h):
        ...
        (so it can't turn into an ipympl/inline widget). Renders a static image every time;
        `_emit` decides whether that image is SVG or PNG."""
    ...
    def _emit(self, fig, save, format=None):
        ...
        # SVG by default: vector output stays crisp at any zoom, ships a few times smaller
        # than the base64 PNG it replaces, and GitHub renders it inline in .ipynb outputs.
        # Pass format="png" for raster-heavy scenes (phase portraits, `raster`,
        # `field_complex`), where SVG only wraps an embedded PNG and comes out *larger*.
        fmt = format or "svg"
        try:  # a static image regardless of the active matplotlib backend
            from IPython.display import SVG, Image, display

            buf = io.BytesIO()
            fig.savefig(buf, format=fmt, facecolor=fig.get_facecolor(), dpi=100)
            # IPython's SVG parses the markup and strips the XML declaration/DOCTYPE itself,
            # so raw savefig bytes are fine here -- no preamble surgery needed.
            display(
                SVG(data=buf.getvalue())
                if fmt == "svg"
                else Image(data=buf.getvalue(), format=fmt)
            )
        except Exception:
            return fig
