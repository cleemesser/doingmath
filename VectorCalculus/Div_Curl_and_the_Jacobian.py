...
# spaces. Such a thing has no trace, no eigenvalues and no symmetric part in general. Only when the domain and
...
#
# We then take Spivak's view, from *Calculus on Manifolds*. The important object is not
# $\operatorname{div}$ or $\operatorname{curl}$, but the exterior derivative $d$. This can act on
# different forms built from $F$. $d$ plus a metric and an orientation provides all you need for differentiation.
# It includes as special cases the vector-valued curl and the classical Green, Stokes and Gauss theorems.
#
# Every claim here is checked in code. Numbers are checked with `np.allclose`. Where the claim is an
# identity for *all* fields, it is checked symbolically with SymPy over generic functions. A
# symbolic check of that kind is a proof, not a spot check.
#
...
# ## Numerical toolkit
#
# A finite-difference Jacobian, the three projections, and a pair of maps we will use throughout.
#
...
# takes this section as its starting point and answers that. It covers the spin bivector as
# $\lfloor n/2\rfloor$ rotation planes. It gives the sharper reason $n=3$ is alone. A bivector needs
# one plane and a normal line, which is why $n=5$ does not sneak through. It treats $\mathbf E$ and
# $\mathbf B$ as one bivector in $\mathbb{R}^{1,3}$. It also treats
# $\nabla F = \nabla\cdot F + \nabla\wedge F$ as the single operator behind both. It also settles
# the question this section invites. $\mathbb{R}^7$ does have a cross product, but it is not
# $SO(7)$-equivariant, so it gives no curl.
