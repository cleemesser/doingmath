...
* :meth:`Space3D.landscape` — the **analytic landscape** of a complex ``f``: the surface ``|f(z)|``
  colored by ``arg f(z)`` (reusing :func:`clmmathtools.viz.phase.colorize`), the 3D form of a phase portrait.
"""
...
class Space3D:
    ...
    def landscape(
        ...
        **kw,
        ...
        ``aa=True`` anti-aliases the contour bands over each mesh cell (see :mod:`clmmathtools.viz.phase`);
    ...
    def display(self, *, format=None, interactive=None, vedo_display=None):
        ...
        Pass ``format="png"`` for scenes carrying a dense ``surface`` or ``landscape``,
        where the polygon count makes SVG the larger of the two.
        """
