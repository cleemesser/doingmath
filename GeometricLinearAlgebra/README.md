...
```bash
uv run meson setup build              # once: configure the meson build directory (./build)
uv run meson compile -C build sync    # regenerate every paired .ipynb from its .py source
