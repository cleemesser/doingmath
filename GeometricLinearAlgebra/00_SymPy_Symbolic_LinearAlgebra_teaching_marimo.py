...
def _(mo):
    mo.md(r"""
    # Geometric Linear Algebra 0 — the symbolic companion, made draggable

    The numerical notebooks in this series prove their claims with `np.allclose` on one
    random sample. The SymPy companion proves them **as identities**: symbols go *inside*
    the matrices, and a difference that `simplify`s to a clean $0$ holds for *all* inputs
    at once.

    This marimo translation keeps every one of those proofs and adds a second layer on
    top of them. Each section now comes in two parts:
    ...
    `TangleLatex` renders a KaTeX string in which every `\tangle{name}` marker becomes a
    draggable number, and publishes the live numbers into a synced `values` dict. It is
    strictly a **renderer plus an input**: it substitutes numbers into LaTeX, it never
    evaluates the math. `\cos \tangle{theta}` displays as `\cos 30`, not as `0.866`.

    Marimo supplies the other half. Dragging mutates `rot.values["theta"]`, marimo re-runs
    exactly the cells that *read* that value, SymPy recomputes, and a derived
    `mo.md(f"$${sp.latex(R)}$$")` block re-renders directly beneath the widget. Widget in,
    symbolic math out.

    Two details make it work well for *this* notebook:

    - **`display: "symbol"`.** A parameter can render as `\theta` until you touch it. Because
      the *same* marker is repeated in all four entries of the rotation matrix and
      `reveal_all_on_drag=True`, a single drag flips the whole symbolic matrix into a
      numeric one at once — the symbol→number transition, animated by hand.
    - **steps chosen to stay exact.** The point of the original notebook is exactness, and a
      slider handing back `0.65000000001` would destroy it. So angles are dragged in
      **15° increments** (`Rational(deg, 180) * pi`) and coefficients in quarters
      (`Rational("0.25")`). SymPy stays exact all the way to the rendered output — you get
      $\tfrac{\sqrt{3}}{2}$, not `0.8660254`.
    """)
    return
...
@app.cell
def _(np, plt, sp):
    # Colors carried over from the vedo original (see CLAUDE.md: named hex constants,
    # reused for visual consistency). vedo's offscreen Plotter is replaced by matplotlib
...
def _(mo):
    mo.md(r"""
    ## 2. Projection, reflection, shear — with a *symbolic* direction

    Each operator carries a defining property, and each becomes an exact identity. Because
    the direction $a=(a_1,a_2)$ stays symbolic, the projection's idempotency is proven for
    **every line through the origin at once**, not for one sampled line.
    """)
    return
...
def _(
    ...
    mo.hstack(
        [
            ...
            mo.md(
                rf"""
    $$\langle u,v\rangle = {sp.latex(dot_x)} \qquad u\wedge v = {sp.latex(wedge_x)}$$
